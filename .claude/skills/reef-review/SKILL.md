...
BACKEND_FILES=$(git diff "$FORK_POINT"..HEAD --name-only -- )
FRONTEND_FILES=$(git diff "$FORK_POINT"..HEAD --name-only -- )
INFRA_FILES=$(git diff "$FORK_POINT"..HEAD --name-only | grep -v '^' | grep -v '^')
SECURITY_FILES=$(git diff "$FORK_POINT"..HEAD --name-only --   | grep -iE 'auth|tenant|security|oauth|token|password|permission' || true)
