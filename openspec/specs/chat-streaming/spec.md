...
### Requirement: 发送/中断按钮状态切换
输入区右下角的按钮根据流式状态切换功能：非流式中为发送按钮（纸飞机图标），流式中为中断按钮（方块图标）。两个状态共享同一按钮位置，使用 `@if/@else` 控制流实现切换。

#### Scenario: 非流式状态下显示发送按钮
- **WHEN** `isStreaming()` 为 false
...
#### Scenario: 用户选择已有会话后 __new__ 被替换
