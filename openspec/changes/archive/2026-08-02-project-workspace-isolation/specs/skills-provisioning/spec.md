...
项目 skills 程序化安装与更新：以 `@deepstorm/cli` 为依赖，服务端通过 `child_process` 以 cwd=项目目录 spawn `deepstorm setup` 在项目创建时安装 tide-* skills 到项目工作区，并在会话开始前基于 FS 版本标记做惰性刷新。调用方经 `SkillsProvider` 接口隔离，将来 deepstorm 若提供编程式 API 只换实现不换调用方。reef-*（语言相关）推迟到代码阶段。
